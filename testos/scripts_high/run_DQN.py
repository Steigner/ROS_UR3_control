...
if __name__ == "__main__":
    ...
    if load:    
        ...
        deepQ = deep_qn.DEEPQ(state_size, action_size, memory_size,learn_start, alpha, gamma, epsilon,epsilon_min,epsilon_discount)
        deepQ.load(path + name + ".h5")
    ...
    for x in range(total_episodes):
        with open (pathdqn_joints,'a') as f:           
            f.write(str(x) + "\n")
        f.close()

        with open (pathdqn_pose,'a') as f2:           
            f2.write(str(x) + "\n")
        f2.close()
        ...
        state = deepQ.getQ(state,state_size)
        ...
        for i in range(500):
            ...
            # env.render()
            action = deepQ.chooseAction(state)
            ...
            nextState = deepQ.getQ(nextState,state_size)

            deepQ.memorize(state, action, reward, nextState, done)
            ...
            if done:
                ...
                h, m = divmod(m, 60)
                print ("EPISODE: "+str(x+1)+" : [alpha: "+str(round(deepQ.learning_rate,2))+" - gamma: "+str(round(deepQ.gamma,2))+" - epsilon: "+str(round(deepQ.epsilon,2))+"] - Reward: "+str(cumulated_reward)+"     Time: %d:%02d:%02d" % (h, m, s))
                break

            if stepCounter > batch_size:
                deepQ.learn(batch_size)
        
        #every 10 iteration save 
        if x % 10 == 0:
            deepQ.save(path + name + ".h5")
