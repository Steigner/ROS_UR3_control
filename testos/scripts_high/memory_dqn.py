...
class Memory:
    ...
    def getMiniBatch(self, size) :
        ...
        for index in indices:
            miniBatch.append({'state': self.states[index],'action': self.actions[index], 'reward': self.rewards[index], 'nextState': self.nextStates[index], 'isFinal': self.finals[index]})
        return miniBatch
    ...
    def getMemory(self, index): 
        return {'state': self.states[index],'action': self.actions[index], 'reward': self.rewards[index], 'nextState': self.nextStates[index], 'isFinal': self.finals[index]}

    def addMemory(self, state, action, reward, nextState, isFinal) :
